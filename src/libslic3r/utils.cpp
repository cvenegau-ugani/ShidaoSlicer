...
{
	// ORCA_BELT: keep "OrcaSlicer" in the G-code "; generated by ..." header
	// even after rebranding SLIC3R_APP_NAME to "OrcaBelt". Moonraker, Fluidd,
	// PrusaLink and other host tools identify the slicer by this exact string
	// — emitting "OrcaBelt" made them fall back to slicer="Unknown" and skip
	// the thumbnail extractor, so Fluidd's job queue showed no previews.
	return std::string("OrcaSlicer " SoftFever_VERSION);
}

std::string header_gcodeviewer_generated()
{
	// Same rationale: downstream tools expect the canonical upstream name.
	return std::string("OrcaSlicer G-code Viewer " SoftFever_VERSION);
}
